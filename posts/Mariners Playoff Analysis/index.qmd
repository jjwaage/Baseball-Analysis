...
one.simulation <- function(s.talent = 0.01){
  ...
  zruns_per_game <- (runs_per_game-mean(runs_per_game))/sd(runs_per_game)
  
  runs_against <- (fieldingstats$RAG)
  zruns_against <-  (runs_against-mean(runs_against))/sd(runs_against)
  
  # simulate talents
  talents <- ((zruns_per_game - zruns_against) / 10)
  ...
  SCH %>% 
    mutate(prob.Home = exp(Talent.Home) /
             (exp(Talent.Home) + exp(Talent.Visitor)) +.04) -> SCH # explain this in blog
