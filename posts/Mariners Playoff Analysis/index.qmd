...
```{r, echo = FALSE}
one.simulation.20 <- function(s.talent = 0.01){
  require(dplyr)
  ...
  # schedule <- rbind(s1, s2, s3, s4, s5, s6, s7, s8, s9)

  schedule1 <- read_excel("schedule.xlsx")
  schedule <- schedule1
  
  # simulate talents
  #talents <- rnorm(30, 0, s.talent)
  talents <- runif(30, min = 0.00001, max = 0.00002)
  ...
  SCH %>% 
    mutate(prob.Home = exp(Talent.Home) /
             (exp(Talent.Home) + exp(Talent.Visitor)) + .04) -> SCH
...
for(j in 1:nrow(all_out)) {
  if (all_out[j,9] != "0") {
    if (all_out[j,1] == "Atlanta Braves") {
      atlplayoffs = atlplayoffs + 1
    }
    if (all_out[j,1] == "Miami Marlins") {
      miaplayoffs = miaplayoffs + 1
    }
    if (all_out[j,1] == "New York Mets") {
      nymplayoffs = nymplayoffs + 1
    }
    if (all_out[j,1] == "Philadelphia Phillies") {
      phiplayoffs = phiplayoffs + 1
    }
    if (all_out[j,1] == "Washington Nationals") {
      wasplayoffs = wasplayoffs + 1
    }
    if (all_out[j,1] == "Chicago Cubs") {
      chcplayoffs = chcplayoffs + 1
    }
    if (all_out[j,1] == "Milwaukee Brewers") {
      milplayoffs = milplayoffs + 1
    }
    if (all_out[j,1] == "Cincinnati Reds") {
      cinplayoffs = cinplayoffs + 1
    }
    if (all_out[j,1] == "Pittsburgh Pirates") {
      pitplayoffs = pitplayoffs + 1
    }
    if (all_out[j,1] == "St. Louis Cardinals") {
      stlplayoffs = stlplayoffs + 1
    }
    if (all_out[j,1] == "Arizona Diamondbacks") {
      ariplayoffs = ariplayoffs + 1
    }
    if (all_out[j,1] == "Colorado Rockies") {
      colplayoffs = colplayoffs + 1
    }
    if (all_out[j,1] == "Los Angeles Dodgers") {
      ladplayoffs = ladplayoffs + 1
    }
    if (all_out[j,1] == "San Diego Padres") {
      sdplayoffs = sdplayoffs + 1
    }
    if (all_out[j,1] == "San Francisco Giants") {
      sfplayoffs = sfplayoffs + 1
    }
    if (all_out[j,1] == "Baltimore Orioles") {
      balplayoffs = balplayoffs + 1
    }
    if (all_out[j,1] == "Boston Red Sox") {
      bosplayoffs = bosplayoffs + 1
    }
    if (all_out[j,1] == "New York Yankees") {
      nyyplayoffs = nyyplayoffs + 1
    }
    if (all_out[j,1] == "Tampa Bay Rays") {
      tbplayoffs = tbplayoffs + 1
    }
    if (all_out[j,1] == "Toronto Blue Jays") {
      torplayoffs = torplayoffs + 1
    }
    if (all_out[j,1] == "Chicago White Sox") {
      cwsplayoffs = cwsplayoffs + 1
    }
    if (all_out[j,1] == "Cleveland Guardians") {
      cleplayoffs = cleplayoffs + 1
    }
    if (all_out[j,1] == "Detroit Tigers") {
      detplayoffs = detplayoffs + 1
    }
    if (all_out[j,1] == "Kansas City Royals") {
      kanplayoffs = kanplayoffs + 1
    }
    if (all_out[j,1] == "Minnesota Twins") {
      minplayoffs = minplayoffs + 1
    }
    if (all_out[j,1] == "Oakland Athletics") {
      oakplayoffs = oakplayoffs + 1
    }
    if (all_out[j,1] == "Houston Astros") {
      houplayoffs = houplayoffs + 1
    }
    if (all_out[j,1] == "Texas Rangers") {
      texplayoffs = texplayoffs + 1
    }
    if (all_out[j,1] == "Los Angeles Angels") {
      laaplayoffs = laaplayoffs + 1
    }
    if (all_out[j,1] == "Seattle Mariners") {
      seaplayoffs = seaplayoffs + 1
    }
  }
}
...
```{r, echo = FALSE}
Num_Playoffs <- c(atlplayoffs, miaplayoffs, nymplayoffs, phiplayoffs, wasplayoffs, chcplayoffs, milplayoffs, cinplayoffs, pitplayoffs, stlplayoffs, ariplayoffs, 
                  colplayoffs, ladplayoffs, sdplayoffs, sfplayoffs, balplayoffs, bosplayoffs, nyyplayoffs, tbplayoffs, torplayoffs, cwsplayoffs, 
                  cleplayoffs, detplayoffs, kanplayoffs, minplayoffs, oakplayoffs, houplayoffs, texplayoffs, laaplayoffs, seaplayoffs)
...
Percent_Playoffs <- Num_Playoffs / 1000
Percent_Playoffs = percent(Percent_Playoffs)

playoffs <- t(rbind(teams, division, Num_Playoffs, Percent_Playoffs))
playoffs = as_tibble(playoffs)

playoffs <- arrange(playoffs, division, desc(Percent_Playoffs))
...
```{r, echo = FALSE}
playoffs %>%  
    mutate(Line = paste("   ",teams, Num_Playoffs, Percent_Playoffs, sep="      ")) %>% 
    dplyr::select(Line) -> out2

  breaks <- c("------------------", "------------------")
  Heading1 <- c("AL East    Number     Percentage", "NL East    Number     Percentage")
  results1 <- cbind(out2[6:10, ], out2[21:25, ])
  names(results1) <- c("A", "B")
  d1 <- rbind(Heading1, breaks, results1)

  Heading2 <- c("AL Cent   Number     Percentage", "NL Cent     Number     Percentage")
  results2 <- cbind(out2[1:5, ], out2[16:20, ])
  names(results2) <- c("A", "B")
  d2 <- rbind(Heading2,  breaks, results2)

  Heading3 <- c("AL West   Number     Percentage", "NL West     Number    Percentage")
  results3 <- cbind(out2[11:15, ], out2[26:30, ])
  names(results3) <- c("A", "B")
  d3 <- rbind(Heading3, breaks, results3)
